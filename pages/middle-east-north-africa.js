...
export default function Home(props) {
  return (
    <div className={styles.container}>
      <Head>
        <title>Middle East & North Africa - The State of Democracy</title>
      ...
      <article
        ...
        style={{
          backgroundImage: `linear-gradient(0deg, rgba(100, 75, 37, 0.74), rgba(100, 75, 37, 0.74))
            ,url("https://upload.wikimedia.org/wikipedia/commons/3/3d/Ghanem_al-Dosari.jpg")`,
        }}
      >
        <p className={styles.description}><a href="/" style={{ textDecoration: 'none' }}>The State of Democracy</a></p>
        <h1 className={styles.title}>
          <span className={styles.titleUnderline} style={{ fontSize: '0.7em' }}>Middle East <span className={styles.and}>&</span> North Africa </span>
        </h1>
      </article>
...
export const getServerSideProps = async () => {
  const countries = await fetch(
    ...
    .then((countries) => filter(countries, country => country.region === 'Middle East & North Africa'));
