...
export default function Home(props) {
  return (
    <div className={styles.container}>
      <Head>
        <title>Western Europe - The State of Democracy</title>
      ...
      <article
        ...
        style={{
          backgroundImage: `linear-gradient(0deg, rgba(100, 75, 37, 0.74), rgba(100, 75, 37, 0.74))
            ,url("https://images.pexels.com/photos/5119451/pexels-photo-5119451.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=750&w=1260")`,
        }}
      >
        <p className={styles.description}><a href="/" style={{ textDecoration: 'none' }}>The State of Democracy</a></p>
        <h1 className={styles.title}>
          <span className={styles.titleUnderline}>Western Europe</span>
        </h1>
      </article>
...
export const getServerSideProps = async () => {
  const countries = await fetch(
    ...
    .then((countries) => filter(countries, country => country.region === 'Western Europe'));
